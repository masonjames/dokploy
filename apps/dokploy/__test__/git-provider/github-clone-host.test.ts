...
const expectCredentialSafeClone = (
	command: string,
	cloneUrl: string,
	expectedHost: string,
) => {
	expect(command).toContain(`-- ${cloneUrl}`);
	expect(command).toContain(`export DOKPLOY_GIT_EXPECTED_HOST=${expectedHost}`);
	expect(command).not.toContain("https://oauth2:gh-token@");
};
...
describe("cloneGithubRepository host", () => {
	...
	it("clones from the Enterprise host, not github.com", async () => {
		...
		expectCredentialSafeClone(
			command,
			"https://acme.ghe.com/acme/web.git",
			"acme.ghe.com",
		);
