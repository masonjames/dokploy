...
test("cleans up preview state when route creation fails", async () => {
	const application = createApplication();
	const domain = {
		domainId: "domain-1",
		host: "preview-my-app-fixedpw-192-0-2-10.sslip.io",
...
test("removes preview routes when domain linking fails after route creation", async () => {
	const application = createApplication();
	const domain = {
		domainId: "domain-1",
		host: "preview-my-app-fixedpw-192-0-2-10.sslip.io",
...
test("keeps the preview deployment row when route removal fails", async () => {
	const application = createApplication();
	vi.mocked(findApplicationById).mockResolvedValue(application as never);
	dbMock.query.previewDeployments.findFirst.mockResolvedValue({
		...previewDeploymentRow,
	...
	vi.mocked(removeWebServerDomain).mockRejectedValueOnce(
		new Error("route cleanup failed") as never,
	);
