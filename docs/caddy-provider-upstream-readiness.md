...
| Caddy dashboard / metrics | Caddy admin endpoint is local-only in the current design, `compileCaddyConfig()` binds admin to `localhost:2019`, `getWebServerDashboardState` reports Caddy dashboard disabled instead of exposing the admin API through Dokploy, and direct dashboard toggle API calls reject while Caddy is active before reading or rewriting legacy Traefik resources. `compileCaddyConfig()` also emits an internal `metrics` HTTP server on `:2020` using Caddy's `metrics` handler so Prometheus can scrape Caddy without exposing the admin API. Active Caddy setup filters reserved `2019/tcp` admin and `2020/tcp` metrics publishing; migration carry-over drops Traefik-only `8080/8082` plus these reserved ports. Dokploy dashboard route rewrites now preserve local Caddy compile settings and restore previous route fragments if the Caddy reload fails. | Locally covered | Prefer no public dashboard in this PR; track any dashboard proxy as a follow-up with auth and network controls. The metrics listener is internal-only and should not be host-published. |
...
- Keep Caddy Prometheus metrics on the internal `:2020` HTTP server generated
  by `compileCaddyConfig()`; do not publish it as an additional host port.
- Treat the metrics listener as private-network but unauthenticated: any workload
  attached to a Caddy-connected Docker network may be able to scrape it unless
  platform network policy blocks that path.
...
| 2026-06-02 | RepoPrompt Deep Plan pass for provider-neutral file-permission copy | Complete | Identified the user-permissions switch for provider-aware web-server file browsing as still using Traefik-only visible copy while intentionally retaining legacy internal permission names for compatibility. |
| 2026-06-02 | `pnpm exec biome check --write --no-errors-on-unmatched --files-ignore-unknown=true apps/dokploy/components/dashboard/settings/users/add-permissions.tsx docs/caddy-provider-upstream-readiness.md`, stale permission-copy `rg`, and `git diff --check` | Passed | Formatter/import check, stale Traefik-only permission-copy search, and whitespace check passed after provider-neutral permission copy. |
| 2026-06-02 | `git diff --name-only upstream/canary...HEAD \| rg '(^\|/)(prompt-exports\|docs/plans\|docs/reviews\|AGENTS\\.md\|CLAUDE\\.md)(/\|$)'` | Passed | No forbidden prompt export, private docs, or agent files are in the current PR diff surface. Local `docs/plans` and `docs/reviews` directories are not tracked in this branch state. |
| 2026-06-02 | RepoPrompt Deep Plan pass for provider-neutral local web-server file nav/search copy | Complete | Identified the sidebar and command palette labels for the provider-aware `/dashboard/traefik` file browser as still Traefik-specific while retaining the legacy route and permission names for compatibility. |
| 2026-06-02 | Stale nav/search copy `rg`, `pnpm exec biome check --write --no-errors-on-unmatched --files-ignore-unknown=true apps/dokploy/components/layouts/side.tsx apps/dokploy/components/dashboard/search-command.tsx docs/caddy-provider-upstream-readiness.md`, `git diff --check`, and PR-surface hygiene scan | Passed | Sidebar and command palette now label the local provider-aware file browser as web-server files while keeping the legacy route and permission names. No forbidden prompt export/private-doc paths were found in the current PR diff surface. |
| 2026-06-02 | Local isolated browser UI QA with temporary Postgres/Redis and Playwright | Partial | Registered a local QA owner and smoke-tested Settings -> Web Server, Requests, local web-server file browser, Caddy read-only artifact browsing, and the Caddy trusted-proxy dialog under Traefik-active and Caddy-active states. No apply, rollback, reload, provider switch, PR update, or live mutation was performed. Application domain modal QA is still pending a local service/domain fixture. |
| 2026-06-02 | RepoPrompt review with git compare `back:3` using per-file artifacts excluding only generated Drizzle snapshot JSON | Complete | Review findings incorporated locally: generic web-server settings updates can no longer toggle Caddy request logs, Caddy request-log toggles now validate the rewritten config before returning success and restore on validation failure, Caddy request-log reads are bounded to the shared retained-line window, cleanup/read retention constants are shared, and Caddy user-agent parsing handles header case defensively. |
| 2026-06-02 | `pnpm --filter=dokploy test --run __test__/caddy/config.test.ts __test__/caddy/request-log-utils.test.ts __test__/caddy/request-log-cleanup.test.ts __test__/caddy/request-logs-router.test.ts` | Passed | 4 files, 37 tests. Covers safe Caddy validate-without-reload config writes, provider-aware request-log toggles, generic schema drift prevention, bounded Caddy log reads for pagination/date-range stats, cleanup retention, and case-insensitive Caddy user-agent headers. |
| 2026-06-02 | `pnpm exec biome check --write --no-errors-on-unmatched --files-ignore-unknown=true <touched files>` | Passed | Formatter/import checks passed for the RepoPrompt request-analytics review slice. |
| 2026-06-02 | `pnpm --filter=dokploy typecheck` and `pnpm --filter=@dokploy/server typecheck` | Passed | App and server package typechecks passed after the RepoPrompt request-analytics review slice. Node v26 produced the expected engine warning because the repo wants Node `^24.4.0`. |
| 2026-06-02 | `pnpm --filter=dokploy test --run __test__/caddy __test__/db/runtime-migration.test.ts` | Passed | 26 files, 169 tests after the RepoPrompt request-analytics review slice. Covers focused Caddy config, setup, request analytics, trusted proxy settings, custom certificates, domain lifecycle, validation, preview cleanup, migration prepare/apply/rollback, upstream preflight, runtime migration, active web-server health checks, dashboard local-only/admin-port behavior, provider-aware application config cache invalidation, and remote web-server file-system inspection. |
| 2026-06-02 | `git diff --check` and PR-surface hygiene scan | Passed | No whitespace errors. No forbidden prompt export, private docs, or agent-file paths were found in `upstream/canary...HEAD`; the local `prompt-exports/` directory from RepoPrompt was removed. |
| 2026-06-02 | RepoPrompt Deep Plan pass for application domain modal UI QA | Complete | Identified an upstream-safe source/API proof slice as the right next step because full React component testing would require new jsdom/testing-library churn. The plan kept browser modal smoke as a separate local-fixture proof item. |
| 2026-06-02 | `pnpm --filter=dokploy test --run __test__/caddy/domain-modal-ui-contract.test.ts __test__/caddy/domain-validation.test.ts` | Passed | 2 files, 9 tests. Covers application domain tab/modal wiring, active Caddy provider resolution, uploaded certificate query/filter/copy, hidden Traefik-only modal fields under active Caddy, and create/update API payload validation for ACME, uploaded certificate, HTTP, and missing custom resolver cases. |
| 2026-06-02 | `pnpm --filter=dokploy test --run __test__/caddy/domain-modal-ui-contract.test.ts __test__/caddy/domain-validation.test.ts __test__/caddy/domain-router-lifecycle.test.ts __test__/caddy/application/domain.test.ts __test__/caddy/application/domain-service.test.ts __test__/caddy/compose/domain.test.ts` | Passed | 6 files, 39 tests. Covers the new modal/source and API schema proof together with Caddy application/compose domain route lifecycle and service-level rollback coverage. |
| 2026-06-02 | `pnpm exec biome check --write --no-errors-on-unmatched --files-ignore-unknown=true <touched files>` | Passed | Formatter/import checks passed for the domain modal/API validation proof slice. |
| 2026-06-02 | `pnpm --filter=dokploy test --run __test__/caddy __test__/db/runtime-migration.test.ts` | Passed | 27 files, 176 tests after the domain modal/API validation proof slice. Covers focused Caddy config, setup, request analytics, trusted proxy settings, custom certificates, domain modal source contracts, domain validation, domain lifecycle, preview cleanup, migration prepare/apply/rollback, upstream preflight, runtime migration, active web-server health checks, dashboard local-only/admin-port behavior, provider-aware application config cache invalidation, and remote web-server file-system inspection. |
| 2026-06-02 | `pnpm --filter=dokploy typecheck` and `pnpm --filter=@dokploy/server typecheck` | Passed | App and server package typechecks passed after the domain modal/API validation proof slice. Node v26 produced the expected engine warning because the repo wants Node `^24.4.0`. |
| 2026-06-02 | `git diff --check` and PR-surface hygiene scan | Passed | No whitespace errors. No forbidden prompt export, private docs, or agent-file paths were found in `upstream/canary...HEAD`; the local `prompt-exports/` directory from RepoPrompt was removed. |
| 2026-06-02 | Local isolated browser modal smoke with temporary Postgres/Redis and Playwright on Chrome | Passed | Registered/logged in a local owner, seeded a local Caddy application and uploaded certificate fixture, opened the application Domains tab, verified Caddy modal copy, HTTPS provider options, uploaded certificate selection, hidden Traefik-only fields, and observed zero forbidden domain/deploy/settings mutation requests. No apply, rollback, reload, provider switch, PR update, or live mutation was performed. |
| 2026-06-02 | RepoPrompt review mode with git compare `back:3` | Complete | Latest review findings incorporated or proven covered: Caddy access-log writer path is covered by the `/etc/caddy` directory mount with explicit standalone/service setup assertions; Caddy request-log normalization now preserves query strings for Traefik parity; date-range Caddy request-log reads remain bounded to the retained-line streaming window. |
| 2026-06-02 | `pnpm --filter=dokploy test __test__/caddy/request-log-utils.test.ts __test__/caddy/setup.test.ts __test__/caddy/request-logs-router.test.ts` | Passed | 3 files, 18 tests. Covers Caddy query-preserving request-log normalization, explicit Caddy setup directory mounts for access logs, and bounded/provider-aware request analytics router behavior. |
| 2026-06-02 | `pnpm --filter=dokploy test __test__/caddy __test__/db/runtime-migration.test.ts` | Passed | 28 files, 179 tests after the browser-modal proof and latest RepoPrompt `back:3` follow-up. Covers focused Caddy config, setup, request analytics, trusted proxy settings, custom certificates, domain modal source contracts, domain validation, domain lifecycle, preview cleanup, migration prepare/apply/rollback, upstream preflight, runtime migration, active web-server health checks, dashboard local-only/admin-port behavior, provider-aware application config cache invalidation, provider-neutral UI contracts, and remote web-server file-system inspection. |
| 2026-06-02 | `pnpm --filter=dokploy typecheck` and `pnpm --filter=@dokploy/server typecheck` | Passed | App and server package typechecks passed after the browser-modal proof and request-log query/mount assertions. Node v26 produced the expected engine warning because the repo wants Node `^24.4.0`. |
| 2026-06-02 | `pnpm exec biome check --write --no-errors-on-unmatched --files-ignore-unknown=true <touched files>`, `git diff --check`, and PR-surface hygiene scan | Passed | Formatter/import checks, whitespace checks, and forbidden-path scan passed. No `prompt-exports/`, private docs, or agent-file paths were found in `upstream/canary...HEAD`; local RepoPrompt exports and scratch containers were removed. |
| 2026-06-02 | RepoPrompt Deep Plan no-mutation production proof pass | Complete | RepoPrompt recommended treating the latest local commit as locally proven unless runtime image provenance matches it. Runtime evidence remains intentionally excluded from this upstream PR surface and tracked out of repo. No provider switch, apply, rollback execution, service update/restart, config write, DNS change, PR update, or production mutation was performed. |
| 2026-06-02 | RepoPrompt Deep Plan pass for remaining provider-neutral UI paper cuts | Complete | Identified the account dropdown quick link and proprietary custom-role permission metadata as still showing visible Traefik-only file-browser copy while the sidebar, command palette, and user-permissions switch were already provider-neutral. |
...
| 2026-06-02 | `pnpm --filter=dokploy test --run __test__/caddy __test__/db/runtime-migration.test.ts` | Passed | 28 files, 183 tests after dashboard route rewrite hardening and review fixes. Covers focused Caddy config, setup, request analytics, trusted proxy settings, custom certificates, domain modal source contracts, domain validation, domain lifecycle, preview cleanup, migration prepare/apply/rollback, upstream preflight, runtime migration, active web-server health checks, dashboard local-only/admin-port behavior, dashboard rewrite rollback/settings compensation, provider-aware application config cache invalidation, provider-neutral UI contracts, and remote web-server file-system inspection. |
| 2026-06-02 | `pnpm --filter=dokploy typecheck` and `pnpm --filter=@dokploy/server typecheck` | Passed | App and server package typechecks passed after dashboard route rewrite hardening and review fixes. Node v26 produced the expected engine warning because the repo wants Node `^24.4.0`. |
| 2026-06-02 | RepoPrompt Deep Plan pass for direct dashboard toggle guard | Complete | Identified that Caddy hides dashboard controls in the UI and reports dashboard disabled, but direct `toggleDashboard` API calls still needed a provider guard before touching legacy Traefik resources. |
...
| 2026-06-02 | RepoPrompt Deep Plan pass for remaining Caddy PR audit | Complete | Identified provider-neutral env-editor wiring, custom-certificate organization-context validation, compose-domain call-site contracts, and tracker cleanup as the latest local-safe PR hardening packet. |
| 2026-06-02 | `pnpm --filter=dokploy test --run __test__/caddy/provider-neutral-ui-contract.test.ts __test__/caddy/certificate-guard.test.ts __test__/caddy/compose-domain-call-sites.test.ts` | Passed | 3 files, 14 tests. Covers provider-neutral web-server env endpoints, fail-closed uploaded Caddy certificate validation without organization context, and source contracts for non-domain-router compose-domain creation paths. |
