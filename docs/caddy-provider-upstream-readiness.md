...
| --- | --- | --- | --- |
| Application domain create under Caddy | `createDomain()` dispatches through `manageWebServerDomain()` for application domains inside the DB transaction. Focused service tests prove provider dispatch and transaction rollback when provider route creation fails. | Locally covered | Add router-level test only if maintainers want end-to-end TRPC proof. |
...
| Preview domains under Caddy | Preview deployment uses `manageWebServerDomain()` for create and `removeWebServerDomain()` for cleanup when a preview domain exists. Focused tests prove the preview app name is set before provider dispatch and cleanup removes by `uniqueConfigKey`. | Locally covered | Add runtime proof after explicit approval if preview deployments are included in the live validation pass. |
| Custom SSL certificates | Caddy domains can now select uploaded certificates. Route intents reference certificate/key files and compiled Caddy JSON emits `apps.tls.certificates.load_files`. | Locally covered | Add runtime proof after explicit approval. Consider a follow-up schema rename because Caddy currently stores the uploaded certificate path in the existing `customCertResolver` field. |
...
| Cloudflare proxy awareness | `compileCaddyConfig()` supports explicit static trusted proxy CIDRs and a Cloudflare preset using static Cloudflare IP ranges plus `CF-Connecting-IP` and `X-Forwarded-For`. Local and remote settings persist the selected mode and Caddy rebuild paths pass it into generated JSON. Default config still trusts no forwarded headers. | Locally covered | Add runtime proof after explicit approval by enabling Cloudflare or static trusted proxies and validating generated `caddy.json` plus route behavior. |
...
| Migration dry-run/apply/rollback | Current PR includes prepare/apply/rollback, runtime preflight, rollback CLI, fail-closed runtime migration tests, and focused migration tests. | Needs final proof | Re-run focused migration suite after new changes and record sanitized results. |
