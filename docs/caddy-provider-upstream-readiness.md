...
| 2026-06-02 | `pnpm --filter=dokploy test __test__/caddy __test__/db/runtime-migration.test.ts` | Passed | 27 files, 176 tests after the browser-modal proof and latest RepoPrompt `back:3` follow-up. Covers focused Caddy config, setup, request analytics, trusted proxy settings, custom certificates, domain modal source contracts, domain validation, domain lifecycle, preview cleanup, migration prepare/apply/rollback, upstream preflight, runtime migration, active web-server health checks, dashboard local-only/admin-port behavior, provider-aware application config cache invalidation, and remote web-server file-system inspection. |
...
| 2026-06-02 | RepoPrompt Deep Plan no-mutation production proof pass plus platform-infra read-only readiness wrapper | Passed | RepoPrompt recommended treating the latest local commit as locally proven unless runtime image provenance matches it. Platform-infra read-only checks passed for the currently promoted Caddy candidate runtime: schema preflight, exact-image dry-run, runtime provider/Caddy posture, rollback CLI check-only, admin-port non-public posture, and route expectations. No provider switch, apply, rollback execution, service update/restart, config write, DNS change, PR update, or production mutation was performed. Platform-specific evidence is recorded outside this upstream PR surface. |
