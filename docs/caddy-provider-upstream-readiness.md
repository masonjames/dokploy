...
- `apps/dokploy/__test__/caddy/trusted-proxy-settings-router.test.ts` covers settings mutation behavior under Traefik, active Caddy rebuild, and rollback if active Caddy reload fails.
...
| 2026-06-02 | `pnpm --filter=dokploy test --run __test__/caddy/trusted-proxy-settings-router.test.ts` | Passed | 1 file, 3 tests. Covers trusted-proxy settings persistence without rebuild under Traefik, active Caddy rebuild with persisted compile settings, and restore-on-reload-failure behavior. |
